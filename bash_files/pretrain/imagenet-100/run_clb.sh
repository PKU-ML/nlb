...
#         do 
# for file in /data/yfwang/SimLFB/solo-learn-outputs/poison_datasets/${dataset}/${method}/gaussian_noise/${dataset}_${method}_rate_1.00_target_*.pt
for file in /data/yfwang/SimLFB/solo-learn-outputs/poison_datasets/${dataset}/${method}/patch/${dataset}_${method}_rate_0.60_target_*.pt
    do
#             # echo $dataset $rate $file
    echo ${i} ${file} 
    sh simclr.sh patch_all " --poison_data ${file}  --use_poison --checkpoint_dir /data/yfwang/SimLFB/solo-learn-outputs/pretrain/${dataset} --gpus 4,5 "
done
